% Cargar los datos
clc; clear; close all;
data = load('curve_tab.txt');
X = data(:,1);
Y = data(:,2);

% Generar más puntos de interpolación para una mejor curvatura
ti = linspace(1, length(X), length(X) * 5); 
X_interp = interp1(1:length(X), X, ti, 'spline');
Y_interp = interp1(1:length(Y), Y, ti, 'spline');

% Calcular derivadas con SplineDerivativeT
[dxdt, dydt, dxxdt2, dyydt2] = SplineDerivativeT(X_interp,Y_interp);

% Calcular curvatura
curvature = abs(dxdt .* dyydt2 - dydt .* dxxdt2) ./ (dxdt.^2 + dydt.^2).^(3/2);

% Aplicar suavizado con una ventana más grande para ver curvatura general
curvature = smoothdata(curvature, 'movmean', 50);

% Normalización por regiones
window_size = 100; % Tamaño de la región
num_points = length(curvature);
curvature_smoothed = curvature;
for i = 1:num_points
    idx_start = max(1, i - window_size);
    idx_end = min(num_points, i + window_size);
    local_max = max(curvature(idx_start:idx_end));
    if local_max > 0
        curvature_smoothed(i) = curvature(i) / local_max;
    end
end

% Graficar la curva con mapa de calor de curvatura
figure;
scatter(X_interp, Y_interp, 10, curvature_smoothed, 'filled');
colormap(jet);
colorbar;
caxis([0 1]);
title('Visualización de la curvatura de la curva');
xlabel('X'); ylabel('Y');
axis equal;

grid on;

function [dxdt, dydt, dxxdt2, dyydt2] = SplineDerivativeT(X,Y)
    t = cumtrapz(sqrt(gradient(X).^2 + gradient(Y).^2));
    fx = spline(t,X);
    fy = spline(t,Y);
    
    d1fx = differentiate(fx);
    d1fy = differentiate(fy);
    d2fx = differentiate(d1fx);
    d2fy = differentiate(d1fy);
    
    ti = linspace(min(t),max(t),length(X));
    
    dxdt = ppval(d1fx,ti);
    dydt = ppval(d1fy,ti);
    dxxdt2 = ppval(d2fx,ti);
    dyydt2 = ppval(d2fy,ti);
    
    dxdt(abs(dxdt)<1e-12) = 0;
    dydt(abs(dydt)<1e-12) = 0;
    dxxdt2(abs(dxxdt2)<1e-12) = 0;
    dyydt2(abs(dyydt2)<1e-12) = 0;
end

function ppdf = differentiate(ppf)
    ppdf = ppf;
    ppdf.order = ppdf.order - 1;
    ppdf.coefs = ppdf.coefs(:,1:end-1) .* (ppdf.order:-1:1);
end
